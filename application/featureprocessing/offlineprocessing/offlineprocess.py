...
class OfflineProcess(movingregion.MovingRegion):
    ...
    def processFootage(self):
        ...
        while self.video_cap.isOpened():
            ...
            #read the curr frame to be processed
            ret,curr_frame=self.video_cap.read()

            if not ret: break

            #when not yet end of input, we resize to required size and convert to gray.
            resized_curr_frame=self.resizeImage(curr_frame)

            moving_area=self.findMovingArea(resized_curr_frame)
            ...
            #check if there is something to track
